...
import { Calculator, Calendar, PieChart, Activity, Zap, TrendingUp, Lightbulb, AlertCircle, CheckCircle, DollarSign, Clock, CalendarDays, RotateCcw, Info, ArrowRight, Layers, Download } from 'lucide-react';
import { InputGroup } from './components/InputGroup';
import { SelectGroup } from './components/SelectGroup';
import { SectionHeader } from './components/SectionHeader';
import { StatBox } from './components/StatBox';
import { PacingVisualizer } from './components/PacingVisualizer';
import { CampaignData, PacingResult, CampaignType } from './types';
import { DEFAULT_CAMPAIGN_DATA } from './constants';
import { calculatePacing, formatCurrency, formatDate, formatPercent } from './utils/calculations';
import { getRecommendation, getTimelineRec, getSpendStatusMsg } from './utils/recommendations';
...
const App: React.FC = () => {
  ...
  const handleInputChange = (field: keyof CampaignData, value: string) => {
    setData(prev => ({
      ...prev,
      [field]: field === 'budget' || field === 'spent' ? parseFloat(value) || 0 : value
    }));
  };
  ...
  const getStatusColor = (status: PacingResult['status']) => {
    switch (status) {
      ...
      case 'trending_high': return 'text-orange-600';
      case 'trending_low': return 'text-orange-600';
  ...
  const getStatusText = (status: PacingResult['status']) => {
    switch (status) {
      case 'over': return 'Critically Overpacing';
      case 'under': return 'Critically Underpacing';
      case 'trending_high': return 'Slightly Overpacing';
      case 'trending_low': return 'Slightly Underpacing';
      default: return 'On Track';
    }
  };

  // Helper to render analyst notes
  const AnalystNote: React.FC<{ children: React.ReactNode }> = ({ children }) => (
    ...
    <div className="w-full max-w-[460px] mx-auto pt-6 pb-12 px-4 flex flex-col items-center justify-center">
      ...
      <div className="mb-6 flex flex-col items-center text-center">
        ...
        <p className="text-[13px] text-gray-500 max-w-[420px] font-normal leading-relaxed">
          Analyze your campaign spend velocity, burn rates, and project your final landing point with precision.
        </p>
      </div>
        ...
        {result ? (
          <div className="animate-fade-in space-y-2">
            ...
            <div className="grid grid-cols-2 gap-3">
              <StatBox label="Progress" value={formatPercent(result.percentElapsed)} subValue={`${result.daysElapsed} of ${result.totalDays} days`} />
              <StatBox label="Remaining" value={`${result.daysRemaining} days`} subValue={`Until ${formatDate(data.endDate)}`} />
            </div>
            <AnalystNote>
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center py-2 border-b border-gray-100">
                ...
                <span className="text-sm font-bold text-gray-900">{formatCurrency(result.idealSpend)}</span>
              </div>
              <div className="flex justify-between items-center py-2 border-b border-gray-100">
                <span className="text-sm text-gray-500">Actual Spend</span>
                <span className="text-sm font-bold text-gray-900">{formatCurrency(result.actualSpend)}</span>
              </div>
              <div className="flex justify-between items-center py-2">
                <span className="text-sm text-gray-500">Variance</span>
                <span className={`text-sm font-bold ${result.variance > 0 ? 'text-red-600' : 'text-green-600'}`}>
            ...
            <div className="grid grid-cols-2 gap-3">
               ...
               <StatBox label="Required Rate" value={`${formatCurrency(result.requiredDailySpend)}/d`} highlight={true} />
            </div>
            <AnalystNote>
              {result.dailyBurnRate > result.requiredDailySpend
                ? `Reduce daily spend by ${formatCurrency(result.dailyBurnRate - result.requiredDailySpend)}. ${getRecommendation(data.campaignType, 'reduceRate')}`
                : `Increase daily spend by ${formatCurrency(result.requiredDailySpend - result.dailyBurnRate)}. ${getRecommendation(data.campaignType, 'increaseRate')}`}
            </AnalystNote>
            ...
            <SectionHeader icon={Activity} title="Projection" className="mt-8" />
             <div className="bg-gray-50 rounded-2xl p-4 border border-gray-200">
                <div className="flex flex-col gap-1">
                  <span className="text-[10px] uppercase font-bold text-gray-400 tracking-wider">Projected Total</span>
                  <span className="text-2xl font-bold text-gray-900">{formatCurrency(result.projectedTotal)}</span>
                  <span className={`text-xs font-medium mt-1 ${result.projectedVariance > 0 ? 'text-red-600' : 'text-green-600'}`}>
                     {result.projectedVariance > 0 ? `Overbudget by ${formatCurrency(result.projectedVariance)}` : `Underbudget by ${formatCurrency(Math.abs(result.projectedVariance))}`}
                  </span>
                </div>
             </div>
             <AnalystNote>
              ...
              <div className="flex gap-3">
                 ...
                 <p className="text-sm text-blue-900 leading-relaxed">
                   {result.status === 'over' || result.status === 'trending_high' ? (
                     <>
                       <strong>Action Item:</strong> Lower daily spend target to <strong>{formatCurrency(result.requiredDailySpend)}</strong> immediately.
                     </>
                   ) : result.status === 'under' || result.status === 'trending_low' ? (
                     <>
                        <strong>Action Item:</strong> Raise daily spend target to <strong>{formatCurrency(result.requiredDailySpend)}</strong> to maximize impact.
                     </>
                   ) : (
                     <>
                       <strong>Action Item:</strong> Maintain current spend rate of <strong>{formatCurrency(result.dailyBurnRate)}/day</strong>.
                     </>
                   )}
                 </p>
              </div>
